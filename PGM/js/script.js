...
const fetchTasks = async () => {
  const response = await fetch('https://control-backend-dvxd.vercel.app/tasks')
...
const addTask = async (event) => {
  ...
  await fetch('https://control-backend-dvxd.vercel.app/tasks', {
    method: 'post',
...
const deleteTask = async (id) => {
  await fetch(`https://control-backend-dvxd.vercel.app/tasks/${id}`, {
    method: 'delete',
  });
...
const createSelect = (value) => {
  const options = `
    ...
    <option value="Tecnico solicitado">Tecnico solicitado</option>
...
const createRow = (task) => {
  ...
  editForm.addEventListener('submit', (event) => {
    ...
    updateTask({ id, title: editInput.value, status, updated_at: editInputDate.value});
  });
